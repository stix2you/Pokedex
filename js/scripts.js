...
pokemonRepository.loadList().then(function () {     // <-- Load the list of Pokémon from the API
   pokemonRepository.getAll().forEach(function (pokemon) {   // <-- Iterate over the list of Pokémon
      pokemonRepository.addListItem(pokemon);                // <-- Add each Pokémon to the DOM as a button
   });
});
